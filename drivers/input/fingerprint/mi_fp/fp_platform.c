...
/* What a reset takes when the device tree does not say. */
#define FP_RESET_TIME_MS_DEFAULT 10
...
int fp_parse_dts(struct fp_device *fp_dev)
{
	...
	if (node) {
		...
		if (of_property_read_u32(node, DTS_RST_HIGH_TIME,
					 &fp_dev->rst_high_time) ||
		    !fp_dev->rst_high_time)
			fp_dev->rst_high_time = FP_RESET_TIME_MS_DEFAULT;
		if (of_property_read_u32(node, DTS_RST_LOW_TIME,
					 &fp_dev->rst_low_time) ||
		    !fp_dev->rst_low_time)
			fp_dev->rst_low_time = FP_RESET_TIME_MS_DEFAULT;
		pr_debug("rst_low time = %u, rst_high time = %u.\n",
			 fp_dev->rst_low_time, fp_dev->rst_high_time);
		...
		of_property_read_string(node, DTS_VENDOR_NAMES,
					&(fp_dev->vendor_names));
		...
		of_property_read_u32_array(node, SENSORLOCATION,
					   fp_dev->position.location, 4);
		/*get wuhb resourece */
		...
		mutex_init(&fp_dev->intr2_lock);

		fp_dev->fd_gpio.work_init = false;
		fp_dev->fw_ipc.work_init = false;
#endif
		/*
		 * Xiaomi's driver asks hwid for the board here and takes the
		 * 3v3 rail from an "fp_3v1_vreg" supply instead on project 11
		 * revision 0.1.  This board is project 2, and its node names
		 * fp_3v3_vreg-supply and nothing else, so that branch cannot
		 * be reached, and the only other thing Xiaomi's driver asks
		 * hwid for is a board revision it prints beside the regulator
		 * voltage, so the dependency it would bring is not taken.
		 */
		fp_dev->vreg_3v3.mRegulator = regulator_get(
			&fp_dev->driver_device->dev, DTS_VOlT_REGULATER_3V3);
		fp_dev->vreg_1v8.mRegulator = regulator_get(
			&fp_dev->driver_device->dev, DTS_VOlT_REGULATER_1V8);
		fp_dev->vreg_gpio.mPwrGpio =
			of_get_named_gpio(node, DTS_VOlT_REGULATER_GPIO, 0);
	} else {
	...
	if (!vreg->IsGpio) {
		if (vreg->mRegulator != NULL) {
			if (!vreg->enable) {
				...
				regulator_set_voltage(vreg->mRegulator, 3300000,
						      3300000);
#endif
				status = regulator_enable(vreg->mRegulator);
				mdelay(1);
				status =
					regulator_get_voltage(vreg->mRegulator);
				vreg->enable = true;
				pr_debug(
					"power on Ldo regulator value is %d!!\n",
					status);
			} else {
		...
		if (vreg->mPwrGpio != -EINVAL) {
			if (!vreg->enable) {
				if (!gpio_is_valid(vreg->mPwrGpio)) {
					...
					gpio_direction_output(vreg->mPwrGpio,
							      0);
					mdelay(1);
					gpio_direction_output(vreg->mPwrGpio,
							      1);
...
/* Hold reset asserted for @low_ms, then release it and settle for @high_ms. */
void fp_hw_reset_delay(struct fp_device *fp_dev, u32 low_ms, u32 high_ms)
{
	pr_debug("rst_low time = %u, rst_high time = %u.\n", low_ms, high_ms);
...
{
	fp_hw_reset_delay(fp_dev, fp_dev->rst_low_time, fp_dev->rst_high_time);
}
...
void fp_local_time_printk(const char *format, ...)
{
	...
	printk(KERN_INFO "xiaomi-fp [%d-%02d-%02d %02d:%02d:%02d.%06lu] %pV",
	       tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
	       tm.tm_min, tm.tm_sec, tv.tv_nsec / 1000, &vaf);
