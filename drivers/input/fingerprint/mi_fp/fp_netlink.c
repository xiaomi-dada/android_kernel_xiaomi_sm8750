...
void fp_netlink_recv(struct sk_buff *__skb)
{
	...
	if (skb->len >= NLMSG_SPACE(0)) {
		...
		nlh = nlmsg_hdr(skb);
		/*
		 * The length test above only proves the header fits. Copying a
		 * fixed sizeof(str) read up to 128 bytes past the payload into
		 * a stack buffer, which was then printed with %s.
		 */
		payload = nlmsg_len(nlh);
		if (payload > sizeof(str) - 1)
			payload = sizeof(str) - 1;
		memcpy(str, NLMSG_DATA(nlh), payload);
		str[payload] = '\0';
		pid = nlh->nlmsg_pid;
...
int fp_netlink_init(struct fp_device *fp_dev)
{
	...
	nl_sk = netlink_kernel_create(&init_net, fp_dev->fp_netlink_num, &cfg);
	if (nl_sk == NULL) {
