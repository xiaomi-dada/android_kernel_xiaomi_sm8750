...
/*
 * pinctrl_select_state() sleeps: pinmux_enable_setting() takes desc->mux_lock.
...
static void xiaomifp_intr2_operation(int SLOT_CUR, int code)
{
	...
	if (code == SYN_REPORT) {
		if (TRACKING_ID > 0) {
			if (Touch_X > fp_dev->position.x1 &&
			    Touch_X < fp_dev->position.x2 &&
			    Touch_Y > fp_dev->position.y1 &&
			    Touch_Y < fp_dev->position.y2) {
...
static void xiaomifp_input_event(struct input_handle *handle, unsigned int type,
				 unsigned int code, int value)
{
	if (type == EV_ABS) {
		if (code == ABS_MT_SLOT) {
			/*
			 * value is whatever the input device reported, and any
			 * userspace uinput device counts. It indexes the int[32]
			 * arrays below and is used as a shift count for SLOT_MAP,
			 * so an unbounded value is a controlled out-of-bounds
			 * write plus undefined shift behaviour. Ignore slots we
			 * cannot represent.
			 */
			if (value < 0 || value >= FP_MAX_SLOTS)
				return;
			fp_dev->position.SLOT_CUR = value;
		} else {
			if (code == ABS_MT_POSITION_X) {
				fp_dev->position
					.touch_x[fp_dev->position.SLOT_CUR] =
					(value / 100);
			}
			if (code == ABS_MT_POSITION_Y) {
				fp_dev->position
					.touch_y[fp_dev->position.SLOT_CUR] =
					(value / 100);
			}
			if (code == ABS_MT_TRACKING_ID) {
				fp_dev->position
					.TRACKING_ID[fp_dev->position.SLOT_CUR] =
					value;
			}
		}
	}
	if (fp_dev->position.enable &&
	    ((code == ABS_MT_TRACKING_ID) || (code == SYN_REPORT))) {
...
int xiaomifp_evdev_init(struct fp_device *_fp_dev)
{
	...
	pr_debug("sensorLocation:%d,%d,%d,%d\n", fp_dev->position.x1,
		 fp_dev->position.y1, fp_dev->position.x2, fp_dev->position.y2);
	return ret;
}
...
void xiaomifp_evdev_remove(void)
{
	...
	cancel_work_sync(&fp_dev->intr2_work);
