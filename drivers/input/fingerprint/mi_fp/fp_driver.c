...
#ifdef FP_ULTRA_QCOM
static int get_events_fifo_len_locked(struct fp_device *fp_dev, int minor_no)
{
	int len = 0;
...
static int setup_ipc_irq(struct fp_device *fp_dev)
{
	...
	rc = request_threaded_irq(fp_dev->fw_ipc.irq, NULL, qbt_ipc_irq_handler,
				  IRQF_ONESHOT | IRQF_TRIGGER_FALLING, desc,
				  fp_dev);
...
static void qbt_gpio_work_func(struct work_struct *work)
{
	...
	state = (gpio_get_value(fp_dev->fd_gpio.gpio) ? QBT_EVENT_FINGER_DOWN :
							QBT_EVENT_FINGER_UP) ^
...
static int setup_fd_gpio_irq(struct fp_device *fp_dev)
{
	...
	rc = devm_gpio_request_one(&fp_dev->driver_device->dev,
				   fp_dev->fd_gpio.gpio, GPIOF_IN, desc);
	if (rc < 0) {
	...
	rc = devm_request_any_context_irq(
		&fp_dev->driver_device->dev, fp_dev->fd_gpio.irq, qbt_gpio_isr,
		IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, desc, fp_dev);
...
static ssize_t set_intr2_fd(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	struct fp_device *fp_dev = &fp;
	dev_info(fp_dev->device, "%s -> %s\n", __func__, buf);

	mutex_lock(&fp_dev->intr2_lock);
	...
	mutex_unlock(&fp_dev->intr2_lock);
	return count;
}

static ssize_t get_intr2_fd(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct fp_device *fp_dev = &fp;
...
static DEVICE_ATTR(intr2, 0600, get_intr2_fd, set_intr2_fd);
...
static DEVICE_ATTR(fingerdown, 0600, get_fingerdown_event,
		   set_fingerdown_event);

static struct attribute *attributes[] = {
#ifdef FP_ULTRA_QCOM
	&dev_attr_intr2.attr, &dev_attr_fingerdown.attr,
#endif
#ifdef CONFIG_SIDE_FINGERPRINT
	&dev_attr_irq.attr, &dev_attr_fingerdown.attr,
#endif
	NULL
};
...
static long fp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	struct fp_reset_time reset_time;
	char vendor_name[10];
	...
	case FP_IOC_RESET_TIME_MS:
		pr_debug("FP_IOC_RESET_TIME_MS  ======\n");
		if (copy_from_user(&reset_time, (void __user *)arg,
				   sizeof(reset_time))) {
			pr_err("Failed to copy reset time from user\n");
			retval = -EFAULT;
			break;
		}
		if (!reset_time.low_ms || !reset_time.high_ms) {
			pr_err("rst_low_delay = %u, rst_high_delay = %u\n",
			       reset_time.low_ms, reset_time.high_ms);
			retval = -EINVAL;
			break;
		}
		fp_hw_reset_delay(fp_dev, reset_time.low_ms,
				  reset_time.high_ms);
		break;
	...
	case QBT_ENABLE_FD:
		...
		if (fp_dev->is_wuhb_connected && !fp_dev->fd_gpio.irq_enabled) {
...
static int fp_open(struct inode *inode, struct file *filp)
{
	...
	if (minor_no == MINOR_NUM_MIFP) {
		...
		fp_dev = container_of(inode->i_cdev, struct fp_device,
				      fpid_cdev);
...
#ifdef FP_ULTRA_QCOM
static ssize_t fp_read(struct file *filp, char __user *ubuf, size_t cnt,
		       loff_t *ppos)
{
	...
	while (fifo_len == 0) {
		...
		if (wait_event_interruptible(
			    *read_wait_queue,
			    (get_events_fifo_len_locked(fp_dev, minor_no) > 0)))
			return -ERESTARTSYS;
		fifo_len = get_events_fifo_len_locked(fp_dev, minor_no);
	}
	...
	if (minor_no == MINOR_NUM_FD) {
		...
		for (i = 0; i < scratch_buf->num_events; i++) {
			...
			pr_debug("x: %d y: %d timestamp: %lld.%03ld\n",
				 fd_evt->X, fd_evt->Y, fd_evt->timestamp.tv_sec,
				 fd_evt->timestamp.tv_nsec);
		}
		pr_debug("%d FD events read at time %lu uS\n",
			 scratch_buf->num_events,
			 (unsigned long)ktime_to_us(ktime_get()));
		to_copy = sizeof(*scratch_buf);
		num_bytes = copy_to_user(ubuf, scratch_buf, to_copy);
		mutex_unlock(&fp_dev->fd_events_mutex);
	} else if (minor_no == MINOR_NUM_IPC) {
		mutex_lock(&fp_dev->ipc_events_mutex);
		...
		pr_debug("IPC event %d at minor no %d read at time %lu uS\n",
			 ...
			 (unsigned long)ktime_to_us(ktime_get()));
		to_copy = sizeof(fw_event.ev);
		num_bytes = copy_to_user(ubuf, &fw_event.ev, to_copy);
		mutex_unlock(&fp_dev->ipc_events_mutex);
	} else {
		pr_err("Invalid minor number\n");
	}
	/*
	 * copy_to_user() returns the number of bytes it could NOT copy, so
	 * returning it made a successful read return 0, i.e. EOF, after the
	 * kfifo had already been drained -- userspace lost every event it
	 * asked for.
	 */
	if (num_bytes != 0) {
...
static __poll_t fp_poll(struct file *filp, struct poll_table_struct *wait)
{
	struct fp_device *fp_dev;
	__poll_t mask = 0;
	int minor_no = -1;
...
int fp_ultra_dev_register(void)
{
	...
	device = device_create(fp_dev->class, NULL, fp_dev->qbt_fd_cdev.dev,
			       fp_dev, FP_WUHB_DEV_NAME);
	if (IS_ERR(device)) {
		ret = PTR_ERR(device);
	...
	device = device_create(fp_dev->class, NULL, fp_dev->qbt_ipc_cdev.dev,
			       fp_dev, FP_IPC_DEV_NAME);
	if (IS_ERR(device)) {
		ret = PTR_ERR(device);
...
#endif
{
	...
	/* get device no */
	/*
	 * Four minors are used: MIFP, MIFP_ID, IPC and FD. Reserving only one
	 * left the other three unregistered, so the same major/minor could be
	 * handed to another driver.
	 */
	status = alloc_chrdev_region(&fp_dev->devt, 0, MINOR_NUM_COUNT,
				     FP_DEV_NAME);
	if (status < 0) {
	...
	status = cdev_add(&fp_dev->cdev,
			  MKDEV(MAJOR(fp_dev->devt), MINOR_NUM_MIFP), 1);
	if (status) {
	...
	status = cdev_add(&fp_dev->fpid_cdev,
			  MKDEV(MAJOR(fp_dev->devt), MINOR_NUM_MIFP_ID), 1);
	if (status) {
	...
	device = device_create(fp_dev->class, NULL, fp_dev->fpid_cdev.dev,
			       fp_dev, FP_ID_DEV_NAME);
	if (IS_ERR(device)) {
		ret = PTR_ERR(device);
...
err_cdev:
	unregister_chrdev_region(fp_dev->devt, MINOR_NUM_COUNT);
...
static int fp_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (!mutex_trylock(&device_list_lock))
		...
		if (fp_dev->fd_gpio.irq_enabled) {
			enable_irq_wake(fp_dev->fd_gpio.irq);
		}
		if (fp_dev->fw_ipc.irq_enabled) {
...
static int fp_resume(struct platform_device *pdev)
{
	struct fp_device *fp_dev = &fp;
	if (fp_dev->fd_gpio.irq_enabled) {
		disable_irq_wake(fp_dev->fd_gpio.irq);
	}
	if (fp_dev->fw_ipc.irq_enabled) {
