...
/*
 * Reset with the timings userspace hands over rather than the ones the device
 * tree gave.  The argument is two u32 -- how long to hold reset asserted and
 * how long to wait after releasing it -- even though the command encodes the
 * size of one; the HAL builds the number this way and the driver has to match
 * it.
 */
#define FP_IOC_RESET_TIME_MS _IOW(FP_IOC_MAGIC, 44, uint32_t)

struct fp_reset_time {
	u32 low_ms;
	u32 high_ms;
};
...
/* Max multitouch slots tracked; bounds SLOT_CUR, which comes from userspace. */
#define FP_MAX_SLOTS 32

struct fp_position {
	bool enable;
	bool reqIntr2;
	int x1, x2, y1, y2;
	int TRACKING_ID[FP_MAX_SLOTS];
	int SLOT_CUR;
	u_int32_t SLOT_MAP;
	unsigned int location[5];
	int touch_x[FP_MAX_SLOTS];
	int touch_y[FP_MAX_SLOTS];
};
...
/**************************CONFIG_QCOM_ULTRA******************************/
	...
	/*
	 * Serialises intr2 pinctrl changes between the sysfs writer and the
	 * input handler. A mutex, not a spinlock: pinctrl_select_state() takes
	 * desc->mux_lock in pinmux_enable_setting(), so it can sleep.
	 */
