					...
					enum led_brightness brightness)
{
	...
	if (current_ma > flash_led_max_ires_values[fnode->ires_idx]) {
		...
		/*
		 * The working copy, not the value the device tree configured:
		 * overwriting the base leaves nothing to fall back to on the
		 * next strobe.
		 */
...
static void check_current_with_time(struct flash_node_data *fnode, u64 off_time_ms)
{
	...
	if(0 == off_time_ms) {
		...
		for(index = 0; index < fnode->custom_max_cur.num_current; index++) {
			if(off_time_ms <= fnode->custom_max_cur.time_for_currents[index]) {
				break;
			}
		}
		if(index == fnode->custom_max_cur.num_current) {
			pick_max_current = fnode->custom_max_cur.always_on_max_current;
		} else {
			pick_max_current = fnode->custom_max_cur.max_currents[index];
		}
	}
...
void sort_with_time(struct custom_max_current *currents)
{
	...
	for(i = 1; i < currents->num_current; i++) {
		int j = i;
		int temp_current = currents->max_currents[i];
		int temp_time = currents->time_for_currents[i];
